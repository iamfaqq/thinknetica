class Menu
  ...
  def choice
    loop do
      puts  'Выберите действие
         ...
         13. Просмотреть список вагонов у поезда. 14. Занять место или объем в вагоне'
  ...
  def add_train_part
    ...
    if @trains_all[train].type == 'пассажирский'
      ...
      @trains_all[train].part_add(PassengerPart.new(seat))
      puts 'Вагон добавлен'
    elsif @trains_all[train].type == 'грузовой'
      puts 'Укажите вместимость вагона'
      volume = gets.chomp.to_i
      @trains_all[train].part_add(CargoPart.new(volume))
      puts 'Вагон добавлен'
    else
      puts 'Выберите поезд'
      trains_list
  ...
  def occupy_space_part
    ...
    if @trains_all[train].type == 'грузовой'
      puts 'Укажите занимаемый объем'
      volume = gets.chomp.to_i
      @trains_all[train].parts_list[part].occupy_space(volume)
    else
      @trains_all[train].parts_list[part].occupy_space
    end
  end
  ...
  def train_parts_list(train)
    @trains_all[train].parts_list.each.with_index(1) do |part, index|
      puts "#{index}: #{part.type}: #{part.volume}."
    end
  end
  end
